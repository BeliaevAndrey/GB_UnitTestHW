...
*/
    ...
    void priceCartProductsSameTypeIsCorrectCalculated() {
        ...
        cart.addProductToCartByID(5);
        // Assert
        assertEquals(cart.getTotalPrice(), 170 * 2 + 15 * 2);
    ...
    void whenChangingCartCostRecalculationIsCalled() {
        ...
        assertEquals(cart.getTotalPrice(), 170 + 15);
    }
    ...
    void quantityProductsStoreChanging() {
        ...
        Cart cart = new Cart(shop);
        cart.addProductToCartByID(5);
        assertEquals(shop.getProductsShop().get(4).getQuantity(), 9);
        cart.addProductToCartByID(5);
        assertEquals(shop.getProductsShop().get(4).getQuantity(), 8);
    ...
    void lastProductsDisappearFromStore() {
        ...
        while (shop.getProductsShop().get(2).getQuantity() > 0)
            cart.addProductToCartByID(3);
        // Assertions
//        assertThatThrownBy(() -> cart.addProductToCartByID(3)).isInstanceOf(IllegalStateException.class);
        cart.addProductToCartByID(3);
        System.setOut(old);
        assertEquals("Этого товара нет в наличии\n", output.toString());

//        assertThat(shop.getProductsShop().get(2).getQuantity()).isEqualTo(0);
    }
...
//        or
        ...
        assertThatThrownBy(() -> cart.addProductToCartByID(num))
                ...
                .hasMessage("Не найден продукт с id: " + num);
    }
    ...
    void incorrectProductRemoveCausesException() {
        ...
        cart.addProductToCartByID(1);
        cart.addProductToCartByID(1);
        assertThatThrownBy(() -> cart.removeProductByID(2)).hasMessage("В корзине не найден продукт с id: 2");
    }
    ...
    @Disabled
    void priceCarIsCorrectCalculatedExt() {
        // Arrange (Подготовка)
        Shop shop = new Shop(getStoreItems());
        Cart cart = new Cart(shop);
        // Act (Выполнение)
